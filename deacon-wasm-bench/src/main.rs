...
fn main() {
    ...
    if args.len() < 3 {
        eprintln!(
            "Usage: {} <index.idx> <reads.fa|fq[.gz]> [chunk_size_kb] [iterations]",
            args[0]
        );
        std::process::exit(1);
    }
    ...
    let reads_data = fs::read(reads_path).expect("failed to read reads file");
...
fn run_filter(
    ...
    let mut session = BenchFilterSession::new(
        index,
        k,
        w,
        deplete,
    ...
    for chunk in data.chunks(chunk_size) {
        session.push_chunk(chunk).expect("filter chunk failed");
    }
    session.finish().expect("filter finish failed");

    session.stats
}

struct BenchFilterSession<'a> {
    index: &'a MinimizerSet,
    k: u8,
    w: u8,
    deplete: bool,
    abs_threshold: usize,
...
impl<'a> BenchFilterSession<'a> {
    ...
    fn push_chunk(&mut self, chunk: &[u8]) -> Result<Vec<u8>, String> {
        let decompressed;
        ...
        let records = self.parser.push_chunk(input)?;
    ...
    fn finish(&mut self) -> Result<Vec<u8>, String> {
        ...
        if let Some(decoder) = self.gz_decoder.take() {
            ...
            if !remaining.is_empty() {
                let records = self.parser.push_chunk(&remaining)?;
                for record in records {
                    self.process_record(
                        &record.header,
                        &record.seq,
...
impl SeqChunkParser {
    ...
    fn scan_fasta(&mut self) -> Result<Vec<SeqRecord>, String> {
        ...
        loop {
            ...
            let header_start = search_from + 1;
...
mod tests {
    ...
    fn fasta_long_sequence_spanning_chunks() {
        let seq_line = "ACGTACGT".repeat(125);
